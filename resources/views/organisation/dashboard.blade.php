...
<p class="alert">Your organisation is verified as a volunteer centre. <a
    href="{{ route('organisation.single', ['slug' => $organisation->slug] ) }}">View your page here</a></p>
</div>
<form method="POST" action="{{ route('organisation.updateCovid')}}" class="card-body">
  @csrf
  <label for="covid_description">Overview</label>
  <textarea name="covid_description" id="covid_description"
    placeholder="You can optionally enter general information to appear on your page here.">{{ $organisation->covid_description }}</textarea>
...
<ul class="list-group">
  ...
  <li class="list-group-item">
    ...
    <div class="list-group-buttons">
      @if($need->enquiries->count())
      <a class="button button__small button__ghost"
        href="{{ route('need.enquiries', ['hashid' => $need->hash()]) }}">View enquiries</a>
      @endif
      <a class="button button__small button__ghost"
        href="{{ route('need.delete', ['hashid' => $need->hash()]) }}">Delete</a>
...
<div class="card opportunity-list">
  ...
  <div class="card-body centered">
    <p>Use the link below to open the ID card maker which allows you to create volunteer cards for any of your
      volunteers.</p>
    ...
    <li class="list-group-item">
      ...
      <div class="list-group-buttons">
        <a class="button button__small button__ghost" href="/opportunity/renew/{{$opportunity->hash() }}">Renew</a> <a
          class="button button__small button__ghost" href="/opportunity/delete/{{$opportunity->hash() }}">Delete</a> <a
          class="button button__small" href="/opportunity/edit/{{$opportunity->hash() }}">Edit</a>
      </div>
    </li>
    @endforeach
  </ul>
