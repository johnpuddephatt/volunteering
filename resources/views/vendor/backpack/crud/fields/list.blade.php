        ...
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'">
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr ng-repeat="item in items track by $index" class="array-row">
                    ...
                    <td>
                        <input class="form-control input-sm" type="text" ng-model="item.text">
                    </td>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            window.angularApp = window.angularApp || angular.module('backPackListApp', ['ui.sortable'], function($interpolateProvider){
                    ...
                    if( typeof $scope.items != 'undefined' ){
                        ...
                        if( typeof $scope.field != 'undefined'){
                            ...
                            for(var i = 0; i < $scope.items.length; i++){
                                $scope.output.push($scope.items[i].text);
                            }

                            $scope.field.val( $scope.items.length ? JSON.stringify($scope.output) : null );
